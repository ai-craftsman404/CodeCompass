...
import { resolveAllConflicts, applyPrecedenceMatrix, validateConflictResolution } from './conflict-resolver';
/**
    ...
    return rules.filter(rule => {
        ...
        for (const [varName, expectedValues] of Object.entries(contextVars)) {
            ...
            if (Array.isArray(contextValue)) {
                ...
                if (!expected.includes(String(contextValue)))
                    return false;
            }
        }
        return true;
 ...
 */
export function shouldSuggestPhasing(context, scanResults) {
...
function getResourceScore(constraint) {
    const scores = {
        'severe': 1.0,
        'moderate': 0.6,
        'standard': 0.3,
 ...
 */
export function determinePhasedRecommendations(resolved, shouldPhase) {
    if (!shouldPhase) {
 ...
 */
export function renderRecommendation(rule, phaseNum, context) {
    return {
        ruleId: rule.id,
 ...
 */
export async function generateRecommendations(projectPath, context, scanResults, rulesDir) {
